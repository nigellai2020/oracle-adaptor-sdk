...
const Bin = require("../../bin/contracts/OSWAP_OracleChainlinkPriceGuardGeneric.json");

export class OSWAP_OracleChainlinkPriceGuardGeneric extends Contract{
    constructor(wallet: Wallet, address?: string){
        super(wallet, address, Bin.abi, Bin.bytecode);
    }
    deploy(params:{weth:string,wethPriceFeed:string,tokens:string[],pricefeeds:string[],factory:string,maxValue:number|BigNumber,deviation:number|BigNumber,returnAmmPrice:boolean}): Promise<string>{
        return this._deploy(params.weth,params.wethPriceFeed,params.tokens,params.pricefeeds,params.factory,Utils.toString(params.maxValue),Utils.toString(params.deviation),params.returnAmmPrice);
    }
