...
const Bin = require("../../bin/OSWAP_OracleChainlinkPriceGuardFiatBinanceTestnet.json");

export class OSWAP_OracleChainlinkPriceGuardFiatBinanceTestnet extends Contract{
    constructor(wallet: Wallet, address?: string){
        super(wallet, address, Bin.abi, Bin.bytecode);
    }
    deploy(params:{wbnb:string,busd:string,usdt:string,factory:string,maxValue:number|BigNumber,deviation:number|BigNumber,useAmmPrice:boolean}): Promise<string>{
        return this._deploy(params.wbnb,params.busd,params.usdt,params.factory,Utils.toString(params.maxValue),Utils.toString(params.deviation),params.useAmmPrice);
    }
